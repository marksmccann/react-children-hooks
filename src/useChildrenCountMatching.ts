import { useMemo, type ReactElement, type ReactNode } from "react";

import childrenToElements from "./childrenToElements";
import type { QueryOptions } from "./types";

/**
 * Returns the number of direct child elements that satisfy the provided predicate.
 *
 * @param children The React children value to inspect.
 * @param predicate A predicate that is called with each direct child element to determine whether it matches.
 * @param options Optional query metadata used to configure how child elements are inspected.
 * @returns The number of direct child elements that satisfy the provided predicate.
 */
function useChildrenCountMatching<T extends ReactElement>(
    children: ReactNode,
    predicate: (element: ReactElement) => element is T,
    options?: QueryOptions
): number;
function useChildrenCountMatching(
    children: ReactNode,
    predicate: (element: ReactElement) => boolean,
    options?: QueryOptions
): number;
function useChildrenCountMatching(
    children: ReactNode,
    predicate: (element: ReactElement) => boolean,
    options?: QueryOptions
): number {
    return useMemo(
        () =>
            childrenToElements(children, options).reduce(
                (count, element) => (predicate(element) ? count + 1 : count),
                0
            ),
        [children, options, predicate]
    );
}

export default useChildrenCountMatching;
